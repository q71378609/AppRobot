...
import cv2
import numpy as np
#from myclass.qmmfxs import qmmfxs
class device:
        ...
        for i in out:
            if s == i.split('\t')[0]:
                ...
                for i in self.config.keys():
                    exec('from myclass.'+i+' import '+i)
                    eval('self.apptasks.append('+i+'(i,self))')
                return
            #devices.append(i.split('\t')[0])
        raise Exception("指定设备不存在...")

    def ADB(self,cmd):
        result = subprocess.Popen('adb.exe -s '+self.name+' '+cmd,shell=True,stdout=subprocess.PIPE)
        out, err = result.communicate()
        # return out.replace(b'\r\r\n', b'\n') #Android6及以下
        return out.replace(b'\r\n', b'\n') #Android7及以上
    
    def get_wm_size(self):
        re = self.ADB('shell wm size')[15:].decode('utf-8').split('x',1)
        self.wm_x,self.wm_y = int(re[0]),int(re[1])

    def get_sc(self):
        self.screenshot = cv2.imdecode(np.frombuffer(self.ADB('shell screencap -p'), np.uint8), cv2.IMREAD_COLOR)
    ...
    def tap(self,xy):
        self.ADB('shell input tap {} {}'.format(xy[0],xy[1]))
